...
- `/task` - 查看自己的当前或最近一次后台任务及进度。
- `/cancel` - 取消自己的当前后台任务，也可使用任务消息中的取消按钮。

### Opportunity Monitor 命令

- `/addop <asset_code> <benchmark_code> [min_score]` - 添加机会监控；例如 `/addop 515180 000922 70`。
- `/delop <id>` - 确认后删除机会监控及其历史快照。
- `/oplist` - 分页浏览规则卡片，直接查询、暂停/恢复、修改阈值和删除。
- `/opon <id>` / `/opoff <id>` - 开启 / 关闭机会监控。
- `/opcheck [id]` - 查询评分摘要，点击“完整明细”展开同次计算的快照；估值和国债数据遵守缓存 TTL。按 ID 可查询暂停规则，不带 ID 时只查询启用规则。
- `/opthreshold <id> <score>` - 将规则告警阈值改为 0–100 的任意分数（可含小数），保留监控状态和历史。

修改阈值本身不发送告警，后续仍遵循跨阈值或等级升级的触发条件。

规则卡片和查询按钮只允许原用户在仍具有白名单权限时操作。卡片显示最近一次观测的评分；手动查询后返回新的摘要，包含分项评分、数据质量和数据日期。批量查询单条失败不影响后续规则，可点击“重试本条”。展开完整明细读取已持久化的原始快照，不重新请求行情；删除规则后，对应的旧明细按钮会提示结果已删除。

### 后台任务

手动查询、添加规则、恢复监控及管理员代理状态查询在后台执行，慢数据源不会占住其他命令的处理队列。手动后台任务最多同时运行 2 个，排队和运行合计最多 20 个；每位用户同时只允许一个耗时任务，重复提交会提示等待或取消。暂停、修改阈值、删除确认和帮助等轻量操作仍直接处理。

任务消息显示当前阶段及取消入口。取消会停止后续处理，不再保存或推送未完成部分的规则和机会快照；已完成的结果及历史数据缓存保留。已经进入工作线程的数据源调用无法被协程取消强制终止，可能继续运行。任务记录仅保留在内存中，服务重启后不自动续跑；旧取消按钮不会影响新任务。

编辑、暂停或删除规则会使其正在计算的旧结果失效，旧任务不会覆盖新的规则状态。添加规则完成初始评分后才写入数据库，取消不会留下尚未完成的规则；恢复监控也会在写入前重新核对规则状态。升级时会为规则增量增加 `revision` 字段，已有规则及快照继续保留。
...
创建规则后会显示盘中告警、每日简报的实际开关和发送安排。未订阅简报时，可点击“开启每日简报”；只有创建该规则的白名单用户可操作该按钮。两种推送均未启用时会明确提示“当前仅支持手动查询”。管理员未配置有效简报时间时不会显示启用按钮。

实时行情最多允许落后 5 分钟；午休和收盘后分别以 11:30、15:00 为新鲜度参照，非交易时段报价不参与技术评分。过期或不可用的报价会回退到已确认的 qfq 历史收盘，并注明原因和历史价格日期。每日简报在实时行情失败时仍尝试生成降级结果，单条规则失败会显示重试命令，不影响其他规则。

盘中等级升级仅在超过当天已通知的最高等级时才能突破普通告警限制；回落后重返已通知等级不会重复发送升级告警。当天最高等级从持久化告警快照读取，重启后仍有效。
