...
Detail requests use `https://api.xueqiu.com/statuses/show.json` and HTTP 404 leaves a `post-ID.statuses-show.not-found` negative-cache marker. Later online and offline runs reuse that marker without requesting the deleted post again; `--refresh` retries it and removes the marker after a successful response. If the run is interrupted with Ctrl-C, fetched cache files are retained and the same command can be rerun to continue.
