...
class TestCalculateRsiWilder:
    """Tests for the explicitly implemented recursive Wilder RSI."""

    def test_normal_calculation(self):
        """正常价格序列应返回合理 RSI 值。"""
        prices = pd.Series([44.0, 44.34, 44.09, 43.61, 44.33, 44.83, 45.10, 45.42, 45.84])
        rsi = calculate_rsi_wilder(prices, period=6)
    ...
    def test_flat_prices(self):
        """Flat prices are neutral rather than an all-gain signal."""
    ...
    def test_different_periods(self):
        ...
        rsi_6 = calculate_rsi_wilder(prices, period=6)
        rsi_14 = calculate_rsi_wilder(prices, period=14)
    ...
    def test_empty_series_returns_none(self):
        """空序列应返回 None。"""
        rsi = calculate_rsi_wilder(pd.Series(dtype=float), period=6)
        assert rsi is None

    def test_single_value_returns_none(self):
        """单个值应返回 None。"""
        rsi = calculate_rsi_wilder(pd.Series([42.0]), period=6)
        assert rsi is None

    def test_recursive_wilder_fixture(self):
        prices = pd.Series([10, 11, 10, 12, 11, 13, 12, 14, 13, 15], dtype=float)
        assert calculate_rsi_wilder(prices, period=3) == 75.62
